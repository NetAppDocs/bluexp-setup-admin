...
Where you install the Connector determines which BlueXP services and features are available. For example, the Connector must be installed in the cloud if you want to deploy and manage Cloud Volumes ONTAP. link:concept-modes.html#private-mode[Learn more about private mode].
