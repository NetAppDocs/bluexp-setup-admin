...
[.lead]
BlueXP offers multiple _deployment modes_ that enable you to use BlueXP in a way that meets your business and security requirements. _Standard mode_ leverages the BlueXP software as a service (SaaS) layer to provide full functionality, while _restricted mode_ and _private mode_ are available for organizations that have connectivity restrictions.
...
.7+| *Features*
| BlueXP identity and access management | No | No
