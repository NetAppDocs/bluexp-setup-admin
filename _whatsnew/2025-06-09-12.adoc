...
The following new features are available within BlueXP IAM.

==== Multi-factor authentication (MFA) for local users
Local users (non-Federated and not associated with a NetApp Support Site account) can enable multi-factor authentication. Administrators can manage users MFA settings such as temporarilty disable or remove them when needed.
