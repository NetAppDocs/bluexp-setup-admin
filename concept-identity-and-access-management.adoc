...
When you create or discover a resource, the resource is associated with the currently selected project. You can then associate the resource across different levels of your resource hierarchy: with one or more folders or with one or more projects. 

For example, you might associate a Cloud Volumes ONTAP system with all projects in your organization or with just a single project. How you associate a resource depends on your organization's needs.

==== When to associate a resource with a folder

An organization admin might associate a resource with a folder so that the folder or project admin can then associate that resource with the appropriate projects that reside in the folder.
