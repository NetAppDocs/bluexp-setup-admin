...
BlueXP prompts you to create a new organization when you sign up.
...
Associating a resource with a folder doesn't automatically make it accessible to all projects in the folder. The _Folder or project admin_ decides which projects can access the resource.. After making that decision, the admin can then associate the resource with the right projects.
...
Organization::
When you grant a member an access roles at the organization level, all folders, projects, and resources in the organization inherit those roles. That means the member has permissions to everything in the organization.
...
You might make a Connector available for another project in these cases:
