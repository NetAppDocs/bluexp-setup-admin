...
NetApp supports service provider-initiated (SP-initiated) SSO only. You need to first configure the identity provider to trust BlueXP as a service provider. Then, you can create a connection in BlueXP that uses your identity provider's configuration.
