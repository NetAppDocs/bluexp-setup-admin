...
////
=== Update the Connector proxy 
If the proxy configuration for your network changes, your Connector might lose access to the internet. For example, if someone changes the password for the proxy server or updates the certificate. In this case, you'll need to access the Connector host directly and update the settings. 
