...
If your corporate policies require you to use a transparent proxy server for all communication to the internet, then you can configure a Connector to use that proxy server. A transparent proxy is a proxy server that intercepts network traffic without requiring any configuration on the client side.

You can configure a Connector to use a transparent proxy server at any time, even if you didn't configure the Connector to use a proxy server during installation. 
