...
+
The key pair that BlueXP uses to generate the CSR is stored internally on the Connector's keystore. BlueXP retrieves the same key pair (private key) when you install the certificate on the Connector.
