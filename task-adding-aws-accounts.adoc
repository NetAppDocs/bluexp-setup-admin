...
[TIP] 
.Can I move Cloud Volumes ONTAP systems to a different AWS subscription?
==== 
No, it's not possible to move Cloud Volumes ONTAP systems to a different AWS subscription. You can replace the AWS subscription that's associated with your credentials, but doing that only affects new Cloud Volumes ONTAP systems that you create. 
====
...
+
NOTE: Replacing an existing subscription doesn't change the AWS subscription for any existing Cloud Volumes ONTAP working environments. Only new Cloud Volumes ONTAP working environments will be associated with the new subscription.
...
The following questions are related to AWS credentials and subscriptions.
