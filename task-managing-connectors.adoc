...
Starting in March 2024, BlueXP now supports the Amazon EC2 Instance Metadata Service (IMDSv2) with the Connector instance and Cloud Volumes ONTAP instances (including the mediator for HA deployments). All prior deployments used IMDSv1. If your security policies require IMDSv2, you can configure any existing Connector instance to use version 2.
