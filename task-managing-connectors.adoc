...
== Require the use of IMDSv2 on the Connector EC2 instance

Starting in March 2024, BlueXP now supports the EC2 Instance Metadata Service (IMDSv2) with the Connector instance in AWS. All prior deployments used IMDSv1. If your security policies require IMDSv2, you can configure any existing Connector instance to use version 2.
...
. Use the `aws ec2 modify-instance-metadata-options` command to require the use of IMDSv2 and to change teh PUT response hop limit to 3.
+
Example
+
[source,awscli]
...
+
NOTE: Setting the `http-tokens` parameter to required means IMDSv2 is required. When `http-tokens` is required, you must also set `http-endpoint` to enabled.

.Result

The Connector instance is now configured to use IMDSv2.
