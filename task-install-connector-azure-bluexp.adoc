...
NOTE: This custom role is unique for when you install a Connector from BlueXP. If you deploy a Connector from the Azure marketplace, you will need to create a different custom role. link:task-install-connector-azure-manual.html[Learn about manually installing a Connector from Azure marketplace.^]
