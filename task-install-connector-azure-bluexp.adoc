...
[.lead]
You can create a Connector in Azure directly from BlueXP. To create a Connector in Azure from BlueXP, you need to set up your networking, prepare an Azure role to use to deploy the Connector, and then deploy the Connector.
...
== Step 2: Create a custom role to use to create the Connector
...
After BlueXP deploys the Connector virtual machine in Azure, it enables a https://docs.microsoft.com/en-us/azure/active-directory/managed-identities-azure-resources/overview[system-assigned managed identity^] on the virtual machine, automatically creates a custom role, and assigns it to the virtual machine. The automatically created role provides BlueXP with the permissions required to manage resources and processes within that Azure subscription. link:reference-permissions-azure.html[Review how BlueXP uses the permissions].

NOTE: If you deploy a Connector from the Azure marketplace (not BlueXP), you need to manually  to create the custom role that the Connector uses to manage resources and processes within Azure. link:task-install-connector-azure-manual.html[Learn about manually installing a Connector from Azure marketplace.^]
