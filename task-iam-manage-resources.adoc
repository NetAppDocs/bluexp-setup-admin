...
NOTE: The IAM resources page excludes Amazon FSx for NetApp ONTAP resources because you cannot associate them with an IAM role. View these resources on their respective canvas or from workloads.
...
If you discover a resource using a BlueXP Connector associate the Connector with the project to grant them access. Otherwise, the Connector and its associated resource are not accessible from the BlueXP canvas by members without the _Organization admin_ role.
