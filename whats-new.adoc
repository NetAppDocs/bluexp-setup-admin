...
The Connector is still supported on Ubuntu, CentOS, and RHEL 7 with Docker (we don't support Podman with these operating systems). There's nothing that you need to do if you have an existing Connector on these operating systems.
+ 
If you have an existing Connector running on RHEL 7, we don't support upgrading the operating system to RHEL 8 or 9. If you need to run your Connector on a RHEL 8 or 9 host instead, then you need to set up a _new_ RHEL 8 or 9 host and install Podman. You can then perform a new Connector installation and configure it to discover the working environments that the old Connector was managing.
