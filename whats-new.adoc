...
At this time, the 3.9.38 release is available for standard mode and restricted mode. This release includes the following changes.

==== Support for IMDSv2

BlueXP now supports the EC2 Instance Metadata Service (IMDSv2) with the Connector instance in AWS. IMDSv2 provides enhanced protection against vulnerabilities.

* For new Connector deployments from BlueXP, from the AWS Marketplace, or using https://docs.netapp.com/us-en/bluexp-automation/automate/overview.html[Terraform scripts^], IMDSv2 is enabled by default on the EC2 instance.

* If you launch a new EC2 instance in AWS to manually install the Connector software, IMDSv2 is also enabled by default.
...
+
https://docs.netapp.com/us-en/bluexp-setup-admin/task-managing-connectors.html[Learn how to configure IMDSv2].
...
Proxy server settings for the Connector are now available from the *Manage Connectors* page.

image:https://raw.githubusercontent.com/NetAppDocs/bluexp-setup-admin/main/media/screenshot-manage-connectors.png[A screenshot that shows the Manage Connectors page that is available after selecting the Connector.]
...
=== IMDSv1 requirement for EC2 instances

You should be aware that the Instance Metadata Service Version 1 (IMDSv1) is required on Amazon EC2 instances before you manually install the Connector software in AWS.

https://docs.netapp.com/us-en/bluexp-setup-admin/reference-limitations.html[Learn more about the requirement for IMDSv1 in AWS].
