...
* These newer versions of RHEL require Podman as the container orchestration tool (previous versions of RHEL required Docker Engine, which is no longer supported).

* This support is available for new Connector installations on RHEL hosts running on your premises or in the cloud when using BlueXP in standard mode or restricted mode.

* If you have an existing Connector running on RHEL 7.x with Docker Engine, we don't support upgrading the OS to RHEL 8.x or 9.x with Podman.
+
You need to perform a fresh installation of the Connector on a host running RHEL 8.x or 9.x with Podman.

<link to installation instructions>

<limitations with backup and recovery and ransomware>

At this time, BlueXP backup and recovery has two limitations when using the Podman engine. See Backup and Restore limitations for details. single-file restore and ransomware scanning
https://docs.netapp.com/us-en/bluexp-backup-recovery/reference-limitations.html

<note about default OS on BlueXP and MP installs>
