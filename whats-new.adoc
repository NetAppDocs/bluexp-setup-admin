...
BlueXP now supports the Amazon EC2 Instance Metadata Service (IMDSv2) with the Connector instance and with Cloud Volumes ONTAP instances, if the Connector is managing only Cloud Volumes ONTAP. BlueXP does not support IMDSv2 with a Connector that you use with any other BlueXP service (for example, backup and recovery or classification).
...
Note the following about how IMDSv2 is enabled on the Connector:
