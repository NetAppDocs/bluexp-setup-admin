...
If you created a Connector from BlueXP, it deployed a VM instance in your cloud provider with a default configuration. After you create the Connector, don't switch to a smaller VM instance with less CPU or RAM.
...
BlueXP alerts you when the Connector VM is unhealthy, including disk space, RAM, and CPU issues. This includes disk space issues, RAM usage, and CPU usage. Monitor these notifications in the Notifications Center within BlueXP or configure it to send email alerts. Resolving these issues is your responsibility. 
...
| CPU usage is too high | Create additional Connectors and distribute the workload across multiple Connectors. CPU utilization can vary based your environment, ONTAP workloads, number of Cloud Volumes ONTAP systems, and the data services that you are using.
| RAM usage is too high | Create additional Connectors and distribute the workload across multiple Connectors. RAM utilization can vary based your environment, ONTAP workloads, number of Cloud Volumes ONTAP systems, and the data services that you are using.
