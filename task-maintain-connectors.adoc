...
BlueXP alerts you when the Connector VM's is reaching an unhealthy state. This includes disk space issues, RAM usage, and CPU usage. Monitor these notifications in the Notifications Center within BlueXP or configure it to send email alerts. Resolving these issues is your responsibility. 
...
| CPU usage is too high | Increase the CPU for the Connector VM instance. Or create additional Connectors and distribute the workload across multiple Connectors. For example, if you are using a single Connector to manage multiple Cloud Volumes ONTAP systems, you can create a Connector for each Cloud Volumes ONTAP system. Connector usage varies based your environment.
| RAM usage is too high | Increase the RAM for the Connector VM instance. Or create additional Connectors and distribute the workload across multiple Connectors.
