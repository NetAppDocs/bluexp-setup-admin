...
The credentials for an Azure subscription must be associated with an Azure Marketplace subscription so that you can pay for Cloud Volumes ONTAP at an hourly rate (PAYGO) or through an annual contract, and to use other BlueXP services.
...
* You can associate only one Azure Marketplace subscription with an Azure subscription
...
Yes, you can. When you change the Azure Marketplace subscription that's associated with a set of Azure credentials, all Cloud Volumes ONTAP working environments will be charged against the new subscription.
...
If you have multiple Azure credentials that all belong to different Azure subscriptions, then those credentials can be associated with the same Azure Marketplace subscription or with different marketplace subscriptions.
