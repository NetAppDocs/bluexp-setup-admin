...
When you create a Cloud Volumes ONTAP working environment, you select AWS credentials that are associated with a specific AWS Marketplace subscription. While it's possible to change the AWS subscription that's associated with credentials, doing so only affects new Cloud Volumes ONTAP systems that you create from that point forward. Replacing an existing subscription doesn't change the AWS subscription for any existing Cloud Volumes ONTAP working environments. 
====