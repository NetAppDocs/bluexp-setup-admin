...
When you use BlueXP to deploy the Connector, the Connector creates this role and applies it automatically to itself. The role's permissions are kept up-to-date when the Connector is upgraded. There is no need to create this role for the Connector or update it yourself.
