...
When using BlueXP without a Connector, users are prompted to enter their ONTAP credentials to discover an on-premises ONTAP cluster and to access the Advanced view (ONTAP System manager) of the cluster. Users can choose to save these credentials in BlueXP which means they won't be prompted to enter them each time. User credentials are only visible to the respective user and can be managed from the *User credentials* page. 

When using a Connector, BlueXP does not prompt users to enter their ONTAP credentials to discover clusters. By default, they can also access the Advanced view (ONTAP System Manager) without needed to enter their credentials. However, a BlueXP administrator (with the Organization admin role) can configure BlueXP to prompt users to enter their ONTAP credentials to ensure they have the right permissions to manage ONTAP clusters. When this setting is enabled, users need enter their ONTAP credentials each time as they are not stored in BlueXP.
