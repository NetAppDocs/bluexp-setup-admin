...
.With a Connector
By default, users are not prompted to enter their ONTAP credentials to access the ONTAP System Manager. However, a BlueXP administrator (with the Organization admin role) can configure BlueXP to prompt users to enter their ONTAP credentials. When this setting is enabled, users need enter their ONTAP credentials each time as they are not stored in Bl
