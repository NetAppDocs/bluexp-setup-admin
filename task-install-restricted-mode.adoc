...
If your BlueXP login is associated with an organization that's operating in standard mode, you'll need to sign up with a new BlueXP login or NetApp Support Site credentials that don't belong to an organization. Otherwise, you won't see the option to enable restricted mode on the setup screen.
