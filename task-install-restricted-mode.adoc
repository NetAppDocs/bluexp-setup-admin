...
The person who sets up the BlueXP Connector must log in to BlueXP using a login that doesn't belong to a BlueXP organization that is operating in standard mode.
 
If your BlueXP login is associated with a BlueXP organization that's operating in standard mode, you'll need to sign up with a new BlueXP login or NetApp Support Site credentials that don't belong to a BlueXP organization. Otherwise, you won't see the option to enable restricted mode on the setup screen.
...
.. Enter a name for a new BlueXP account or select an existing restricted mode account.
+
You can select an existing account if your log in is already associated with an existing BlueXP account that is operating in restricted mode.
