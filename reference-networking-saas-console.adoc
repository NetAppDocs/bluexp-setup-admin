...
These endpoints are contacted from a user's computer when completing specific actions from the https://console.bluexp.netapp.com[BlueXP web-based console^] that's provided through the SaaS layer. 

If you manually install the Connector, the endpoints are also contacted from a user's computer when you open a web browser, enter the IP address of the Connector host, and then log in and set up the Connector.

You should also refer to networking requirements for the Connector and for specific BlueXP services. For details, refer to the related links at the end of this page.
